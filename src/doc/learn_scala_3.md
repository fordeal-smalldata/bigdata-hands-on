...
1. 牛顿迭代法求平方根:牛顿迭代法的介绍见[这里](https://www.matrix67.com/blog/archives/361),或者你去别的什么地方找也行.
   ...
   参考答案在`src/main/scala/answer/Answer3.scala`中

2. 数字转中文大写:在经济活动中,我们为了防止恶意人士篡改数据,以及加强数字的抗污损能力,往往会在打印票据的时候用中文大写打印重要的数字,中文大写数字的介绍可以看[百度百科](https://baike.baidu.com/item/%E4%B8%AD%E6%96%87%E6%95%B0%E5%AD%97),现需要您实现一个数字转中文大写的函数.

   按照[材料准备](site:/index.md#习题集)里的提示把习题下载到本地,编辑`src/main/scala/quiz/Quiz4.scala`,更改文件中的`???`为您的答案.在项目根目录下执行`./gradlew run --args='quiz.Quiz4'`,获得以下输出说明习题完成.

   ```scala
   1 的中文大写形式为 壹
   1024 的中文大写形式为 壹千零贰十肆
   50000 的中文大写形式为 伍万
   50010 的中文大写形式为 伍万零壹十
   500019 的中文大写形式为 伍十万零壹十玖
   2147483647 的中文大写形式为 贰十壹亿肆千柒百肆十捌万叁千陆百肆十柒
   ```

   参考答案在`src/main/scala/answer/Answer4.scala`中